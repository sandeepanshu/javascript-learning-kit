...
// have some key differences in terms of scope, hoisting, and reassignment. Here's a brief overview of each:
...
// Also introduced in ES6, const is used to declare constants, whose values cannot be reassigned after initialization.
...
// Local scope
// Variables defined inside a function are in the local scope. And they have a different scope for
// every call of that function. This means that variables having the same name can be used in
// different functions. This is because those variables are bound to their respective functions,
...
// oondition statements are used to decide the flow of execution based on specified 
...
// Program. Create a program to get sum of all digits of a number
...
// 1. print all the prime number up to given number value.
...
// Object.keys() method takes the object that we want to loop over as an argument and returns the elements in an array that 
// contains all properties names (or keys).
...
// Object.entries() gives outputs of an array of arrays that consists of each inner array having two elements. The first element is 
// considered being the property and the second element is the value.
// const array_animals = {
// lion: 1,
// giraffe: 2,
// tiger: 3,
...
// Looping through The Object.values() method works directly opposite to that of Object.key(). Object.values() method functions 
// by returning the values of all properties in the object as an array. 
...
// basically a widely accepted format to exchange data 
// between various application including client-server 
// applications as well and also a great alternative to XML. 
// The file containing JSON objects is saved with the 
// extension .json.

// Difference between JSON Object and Javascript Object:
// Through JSON Object and Javascript Object have a close resemblance as both are key:value pairs there are few things we 
// need to note
// ● Property name/key are always in double quotes " " in JSON.
// ● The keys are any valid string, but the JSON values can only be one of the six data types (strings, numbers, objects, 
// arrays, boolean, null). Unlike in Javascript Objects, the values can be anything, as we saw earlier.
// Application of JSON
...
// • Higher order functions operate on other functions, either by receiving them as arguments or by 
// returning them. It is a function that accepts a function as a parameter or returns a function as the output.
...
// Example 1
// Given an array of integers, create a new array that will have double of each element in the first array 
...
// Example 2
// Given an array of nouns in singular form, create a new array that stores the plural noun of each word in 
// the first array, and log it to the console (assume that the singular nouns can be made plural by adding a 
// 's').
// Solution:
...
// filter function
// The filter() function is used while creating a search bar with a given list of items. The filter() method also creates a new array by executing the passed callback on 
// every element of the array, and keeps it in the resulting array IF and ONLY IF the element passes the Boolean test returned by the callback.
...
// outputs a single value. It does something on every element and keeps a record of the calculations in an 
// accumulator variable and when no more elements are left, it returns the accumulator.
...
// If the optional initialValue is given, the accumulator at the first iteration will be equal to the initialValue and 
// the currentItem will be equal to the first element in the array. Otherwise, the accumulator would be equal to the first 
// item in the input array, and the currentItem will be equal to the second item in the array.
...
// 2. Given an array of numbers, find the sum of every element in the array, starting with 5, and log the result to 
// the console
// Solution:
// var input = [1, 2, 3];
...
// 5
// 6
// The big difference between the map and forEach method is that the map method creates a new array, "mapping" 
// the return value of the callback and create a new array, while the forEach method just iterates over the array.

// sort function
// Sort function is used to sort the array based on the callback function condition and it should sort ascending order 
// by default
// Example
...
// slice function
// The slice() method returns a copy of a portion of an array into a new array selected from start to end (end not 
...
// Destructuring in Javascript is an expression that makes it achievable to unpack values from properties from 
// objects. We can extract the data from objects and assign them to distinct variables. The value that should be 
// unpacked from the sourced variable is defined on the left-hand side.
...
// Pure Function (568)
// • Pure function does not change the state of 
// variables out of its scope and it will always return 
// same output if we pass the same input multiple 
// number of times.
// • The above definition is little bit hard to understand, 
// so let me explain this with example.
// • This real time example will show whenever you 
// are studying about our planet earth every time the 
// statement ‘Earth is a planet’ is same.

// Impure Function (570)
// • Impure function will change the state of variables out of its scope and it will always return different output if 
// we pass the same input multiple number of times.
// • The above definition is little bit hard to understand, so let me explain this with example. 
// • This above real time example will show whenever you are studying about pluto in school days we heard that 
// pluto is the 9th planet in our solar system but now pluto is not a planet. It shows that every time it is changing 
// and it has side effects.

// Pure Function (571)						Impure Function
// It has no side-effects				It may have side-effects
// It will return same output if same arguments are passed 
// how many times it executes
// it will return different output if same argument passed on 
// multiple times
...
// Closure
// • Closure is one of the important concept in Javascript. It is widely discussed concept in javascript world and still 
// confusing concept. Let’s understand closure in very simple ways.
...
// o Closure is a function having access to the parent scope => which means a function(Parent function) is returning 
// a function(child function)
// o even after the parent function has closed => the variable which are present in the parent functions are 
// accessible from the child function after called the parent function.

// Higher Order Function
// • Higher Order Function (HOF) are functions that takes other function as 
// argument or return function as a result. I hope you are familiar with 
// Arrays and their functions.
// • Let me take you into deeper in this HOF concept. Arrays function such 
// as map, filter, sort, reduce, forEach and so on are Higher Order function 
// because these functions accept function as an argument and return 
// desired output. (592)
